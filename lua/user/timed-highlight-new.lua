...
local lua_command_string = ":lua require('user.timed-highlight-new').turn_off_highlight_after_expiration()<CR>"
        ...
        vim.schedule(
            function()
                if cmd_type ~= nil and (cmd_type == '/' or cmd_type == '?') then
                    require('user.timed-highlight-new').turn_off_highlight_after_expiration()
                end
            end
        )
    end
